...
public class GetRiotPlayerBasicStoreAspect {
    ...
    private void getStoreAnnotation() {}

    @Around("getStoreAnnotation()")
    public Object findStoreByPlayerName(ProceedingJoinPoint pjp) throws Throwable {
        ...
        RiotPlayerBasicDto riotPlayerBasicDto;

        if (riotRedisService.existsByPlayerBasicName(playerName)) {
            riotPlayerBasicDto = riotRedisService.getRiotPlayerBasicDto(playerName);
            return ApiResponse.of(LOL_PLAYER_FOUND, riotPlayerBasicDto);
        }
        else if (riotRepository.existsByPlayerName(playerName)) {
            riotPlayerBasicDto = RiotPlayer.toRiotPlayerBasicDto(riotRepository.findByPlayerName(playerName));
            return ApiResponse.of(LOL_PLAYER_FOUND, riotPlayerBasicDto);
        }
